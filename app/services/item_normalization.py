...
catalog descriptions (catalog_import.py) and salesman item spans
(match_item.py), so the two sides of every comparison are normalized
identically. Built on top of app.services.normalization.normalize_text
(lowercase/NFC/whitespace/punctuation) rather than reimplementing it.
...
from app.services.normalization import COLOR_SYNONYMS, SIZE_SYNONYMS, normalize_text
...
# the abbreviation. Kept separate from LEBANESE_DICT (a translation
# problem) even though some entries overlap in effect.
...
# the single source of truth in app/services/normalization.py
# (SIZE_SYNONYMS/COLOR_SYNONYMS) rather than duplicating it.
...
def _split_glued(token: str) -> list[str]:
    ...
    signal. Splits on alpha-run/digit-run boundaries so "medium14x6" ->
...
def _expand_dict(tokens: list[str], mapping: dict[str, str]) -> list[str]:
    ...
    never destructive, so information is never erased.
...
def normalize_item_text(text_val: str) -> str:
    """Deterministic normalization pipeline: unicode normalize / lowercase
    / whitespace-collapse / punctuation-strip (via normalize_text), then
    additive abbreviation + Lebanese/Arabizi synonym expansion. Numeric
    pack codes are left untouched by every step here - normalize_text
    already preserves digits, and neither dict below maps anything that
    looks like a number.
    """
